//
//  ConnectedUserModelResp.swift
//  BeachPartner
//
//  Created by Powermac on 7/5/18.
...
struct ConnectedUserModelRespArray: SafeMappable {
    
    var connectedUserModelResp = [ConnectedUserModelResp]()
    ...
    init(_ map: NSArray) throws {
        for model in map {
            ...
            do {
                let respModel = try ConnectedUserModelResp(model as! [String : Any])
                connectedUserModelResp.append(respModel)
            } catch {
                print(error)
            }
        }
...
struct ConnectedUserModelResp:SafeMappable {
