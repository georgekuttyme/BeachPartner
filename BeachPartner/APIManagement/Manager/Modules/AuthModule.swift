...
//                let accRespModel = try ConnectedUserModelArray(jsonDict)
    ...
    public func getAllAddonPlans(sucessResult:@escaping resultClosure,errorResult:@escaping errorClosure) {
        
        let params = [String: String]()
        APIGetClient.doGetRequest.inGetReqForArr(method: ApiMethods.getAddonPlans, params: params, sucess: { (response) in
            APIManager.printOnDebug(response: " Resppp1111 : \(String(describing: response))")
    ...
    public func getUsersActivePlans(sucessResult:@escaping resultClosure,errorResult:@escaping errorClosure) {
        
        let params = [String: String]()
        
        APIGetClient.doGetRequest.inGet(method: ApiMethods.usersActivePlans, params: params, sucess: { (response) in
            let jsonDict = response
            do {
                let accRespModel = try GetUserSubscriptionModel(jsonDict! as! [String : Any])
                sucessResult(accRespModel)
                return
            } catch {
                print("Catched")
